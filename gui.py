...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        #TODO done - Calcular distancia de cada par:
        pairs = self.agent.pairs
        for pair in pairs:
            ...
            if pair.cell1 not in self.agent.forklifts:
                if pair.cell1.column + 1 < state.columns and state.matrix[pair.cell1.line][pair.cell1.column + 1] == constants.EMPTY:#se a célula do lado direito nao for parede & está vazia (nao seja outra estante, pois as estantes têm largura de 2)
                    state.set_forklift(pair.cell1.line, pair.cell1.column + 1)#ir para direita
                else:
                    state.set_forklift(pair.cell1.line, pair.cell1.column - 1)#se nao, ir esquerda
            else:
                state.set_forklift(pair.cell1.line, pair.cell1.column)#se for outro forklift, manter posição


            if pair.cell2 != self.agent.exit: #se proximo destino (proximo produto) NAO for exit, fazer o mesmo que acima
            ...
            # ver melhor caminho possível calculado para o par de células em questão
            pair_solution = self.agent.solve_problem(problem)#vai ver o melhor caminho com a cell1 e cell2 que demos utilizando o grph_search
            self.agent.add_solution_to_pair(pair, pair_solution)
            self.gui.text_problem.insert(tk.END, "Pair: " + str(pair) + " - Solution: " + str(pair_solution) + "\n")
        #TODO done ate aqui
        ...
        self.gui.text_problem.insert(tk.END, "Done!\n")
        self.gui.text_problem.insert(tk.END, "Number of pairs: " + str(len(self.agent.pairs)) + "\n")


class SolutionRunner(threading.Thread): #atualiza a posicao dso forklifts etc na simulacao
    ...
    def run(self):
        self.thread_running = True
        forklift_path, steps = self.best_in_run.obtain_all_path() #obtain_all_path chamado aqui
        ...
        for step in range(steps - 1):
            ...
            for j in range(len(forklift_path)):
                ...
                if step < len(forklift_path[j]) - 1:
                    ...
                    #Todo colocar produtos apanhados a preto:
                    if new_cell.column + 1 < self.state.columns and self.state.matrix[new_cell.line][new_cell.column + 1] == constants.PRODUCT: #added
                        self.state.matrix[new_cell.line][new_cell.column + 1] = constants.PRODUCT_CATCH #added
                    if new_cell.column - 1 >= 0 and self.state.matrix[new_cell.line][new_cell.column - 1] == constants.PRODUCT: #added
                        self.state.matrix[new_cell.line][new_cell.column - 1] = constants.PRODUCT_CATCH #added
